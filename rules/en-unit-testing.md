...
- Repo-specific mock setups and pitfalls (reanimated / gesture-handler ordering, EXPO_OS constant folding, etc.) belong in that repository's own `.rulesync/rules/unit-testing.md` — a non-root fragment carrying the same `globs` as this one, so it loads next to this file and stays out of the always-loaded root rule. Do NOT put them in `general.md`.
